...
from rpac.util.common import tabFocus, serveFile, sendEmail, \
    ReportGenerationException, logError
...
from rpac.model import STATUS_APPROVE, STATUS_NEW, STATUS_UNDER_DEV, \
    STATUS_CANCEL, STATUS_DISCONTINUE, Group, Item, qry, DBSession, \
    FileObject, FileVersion
from repoze.what.predicates import has_permission
from rpac.model.logic import FILE_CHECK_IN, FILE_CHECK_OUT
from rpac.util.excel_helper import getExcelVersion, BBBReport



log = logging.getLogger(__name__)
EMAIL_SENDFROM = 'r-pac-bbb-system@r-pac.com.hk'
EMAIL_SUFFIX = [
                "*" * 80,
                "This e-mail is sent by the r-pac Bed Bath & Beyond system automatically.",
...
class LogicController(BaseController):
    ...
    def index(self , **kw):
        ...
        if kw.get("itemCode", False) : ws.append(Item.itemCode.op("ilike")("%%%s%%" % kw["itemCode"]))
        if kw.get("brand", False) : ws.append(Item.brand.op("ilike")("%%%s%%" % kw["brand"]))
    ...
    def _getReportFilePath(self, templatePath):
        ...
        fileDir = os.path.join(config.get("public_dir"), "bbb", dateStr)
    ...
    def export(self, **kw):
        ...
        formtype = kw.get("formtype", "INDEX")
        if formtype == "PRODUCTION":  # it's production form
            ws.append(Item.status == STATUS_APPROVE)
        ...
        data = []
        for h in  qry(Item).filter(and_(*ws)).order_by(desc(Item.createTime)).all():
            data.append(map(lambda v : unicode(v or ''), [ h.systemNo, h.jobNo, h.itemCode, h.brand, h.developer, h.desc, h.createTime.strftime("%Y/%m/%d %H:%M"),
                                      h.showStatus(), h.approveTime.strftime("%Y/%m/%d %H:%M") if h.approveTime else '',
                                     ]))
        try:
            v = getExcelVersion()
            if not v : raise ReportGenerationException()
            if v <= "2003" :  # version below 2003
                templatePath = os.path.join(config.get("public_dir"), "TEMPLATE", "BBB_REPORT_TEMPLATE.xls")
            else :  # version above 2003
                templatePath = os.path.join(config.get("public_dir"), "TEMPLATE", "BBB_REPORT_TEMPLATE.xlsx")

            tempFileName, realFileName = self._getReportFilePath(templatePath)
            sdexcel = BBBReport(templatePath = tempFileName, destinationPath = realFileName)
    ...
    def save(self, **kw):
        params = {
                  ...
                  "itemCode" : kw.get('f_itemCode', None) or None,
                  "brand" : kw.get('f_brand', None) or None,
                  "developer" : kw.get('f_developer', None) or None,
                  "desc" : kw.get('f_desc', None) or None,
                  }

        if self._is_duplidate(params['jobNo']) :
            flash("Duplicated job number found!", "warn")
            return redirect("/logic/index")
        ...
        try:
            ...
            # send email
            subject = "[BBB] New Job number[%s] is added by Bed Bath & Beyond" % obj.jobNo
            to = self._get_email_users('AE')
            content = [
                       "Dear User:",
                       "New job number is added by Bed Bath & Beyond,please check the below URL to check the job's detail.",
    ...
    def add_new_file(self, **kw):
        ...
        try:
            ...
            if flags:
                ...
                subject = "[BBB] New File(s) are added to job[%s]" % obj.jobNo
                to = self._get_email_users(*flags)
                content = [
                           "Dear User:",
                           "New file(s) are added to job[%s] ,please check the below URL to check the job's detail." % obj.jobNo,
                           "%s/logic/detail?id=%s" % (config.get('website_url', ''), obj.id),
                           "", "Thanks",
                           ]
                self._sendEmail(subject, to, content)
    ...
    def change_status(self, **kw):
        ...
        if status == unicode(STATUS_UNDER_DEV):
            ...
            if obj.status == STATUS_NEW:
                # send email
                subject = "[BBB] Job[%s] is started" % obj.jobNo
                to = self._get_email_users('AE', 'BBB')
                content = [
                           "Dear User:",
                           "The job[%s] is started ,please check the below URL to check the job's detail." % obj.jobNo,
                           "%s/logic/detail?id=%s" % (config.get('website_url', ''), obj.id),
                           "", "Thanks",
                           ]
                self._sendEmail(subject, to, content)
            ...
            try:
                ...
                # send email
                subject = "[BBB] Job[%s] is approved for production" % obj.jobNo
                to = self._get_email_users('PRODUCTION')
                content = [
                           "Dear User:",
                           "The job[%s] is approved for production ,please check the below URL to check the job's detail." % obj.jobNo,
                           "%s/logic/detail?id=%s" % (config.get('website_url', ''), obj.id),
                           "", "Thanks",
                           ]
                self._sendEmail(subject, to, content)
    ...
    def ajaxCheckDuplidate(self, **kw):
        ...
        if not no : return {'flag' : 1 , 'msg' : 'job no could not be blank'}
    ...
    def ajaxShareFile(self, **kw):
        ...
        try:
            ...
            # send email to BBB
            item = qry(Item).get(obj.referto)
            subject = "[BBB] A file of job[%s] is shared with you" % item.jobNo
            to = self._get_email_users('AE', 'BBB')
            content = [
                       "Dear User:",
                       "A file of job[%s] is shared with you ,please check the below URL to check the job's detail." % item.jobNo,
    ...
    def _append(self, fn):
        p = fn.rindex(".")
        if p > -1 : return "%s_Modified%s" % (fn[:p], fn[p:])
        return fn + "_Modified"
    ...
    def checkInFle(self, **kw):
        ...
        try:
            # keep the version
            DBSession.add(FileVersion(file = obj,
                                      fileName = self._append(obj.fileName),
                                      remark = obj.remark, _file_path = obj._file_path,
                                      createTime = obj.createTime, updateTime = obj.updateTime,
                                      createById = obj.createById, updateById = obj.updateById,
                                      ))
            # update info
            ...
            if hasattr(fobj, "filename") : obj.fileName, obj.filePath = self._handle_upload(fobj)
            ...
            if flags:
                if type(flags) != list : flags = [flags, ]
                subject = "[BBB] File is modified to job[%s]" % item.jobNo
                to = self._get_email_users(*flags)
                content = [
                           "Dear User:",
                           "File is modified to job[%s] ,please check the below URL to check the job's detail." % item.jobNo,
                           "%s/logic/detail?id=%s" % (config.get('website_url', ''), item.id),
                           "", "Thanks",
                           ]
                self._sendEmail(subject, to, content)
    ...
    @expose()
    def downloadFromVersion(self, **kw):
        try:
            obj = DBSession.query(FileVersion).get(kw["id"])
